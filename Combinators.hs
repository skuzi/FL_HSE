...
lstrip [] = []
lstrip (c : cs) | T.isWhiteSpace c = lstrip cs
                | otherwise        = c:cs
...
p >>= q = \inp ->
  case p (lstrip inp) of
    Success (r, inp') -> q r (lstrip inp')
    Error err -> Error err
...
number [] = ([], [])
number (c : cs) | T.isDigit c = 
                   let (pred, suff) = number cs in (c : pred, suff)
                | otherwise = 
                   ([], c : cs)
...
ident (c : cs) | T.isAlpha c = 
               let (pred, suff) = takeWord cs in
                  Success (c : pred, suff)
             | otherwise = 
                  Error ("Not a letter" ++ show (c : cs))

takeWord :: String -> (String, String)
takeWord [] = ([], [])
takeWord (c : cs) | (T.isAlpha c || T.isDigit c) = 
                     let (pred, suff) = takeWord cs in (c : pred, suff)
                  | otherwise = 
                     ([], c : cs)
